...
def plot_survival_functions(res, clean_names, event_name, predictor_name, before, train):
    ...
    plt.savefig('figs/{}_survival_{}->{}_{}.png'.format('Before' if before else 'After', predictor_name, event_name, 'train' if train else 'val'))
...
def analyse(data, data_fake, before, train, ENDPOINT, event_name, predictor_name, plot=True):
    ...
    if plot:
        plot_survival_functions(res, clean_names, event_name, predictor_name, before, train)
